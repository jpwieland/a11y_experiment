# ============================================================
#  a11y-autofix — Configuração Completa do Ambiente (Windows)
#  Suporte: Windows 10/11 com PowerShell 5.1+ ou PowerShell 7+
#  GPU: NVIDIA CUDA (via Ollama/vLLM no WSL2)
#
#  Uso (execute como Administrador ou configure a política):
#    .\setup.ps1                  # setup completo
#    .\setup.ps1 -NoModels        # pula pull dos modelos Ollama
#    .\setup.ps1 -NoGpu           # força modo CPU apenas
...
function RunCmd {
    ...
    Write-Log $line
    & $Cmd @Args 2>&1 | Out-File -Append -FilePath $LogFile
}
...
Write-Host ""
Write-Host "*** a11y-autofix — Configuração Completa do Ambiente (Windows) ***" -ForegroundColor Cyan
Write-Host "=" * 60
...
if (-not $PythonExe) {
    Die "Python 3.10+ nao encontrado.
  ...
  OU via conda:  conda create -n a11y python=3.12 && conda activate a11y

  IMPORTANTE: marque 'Add Python to PATH' durante a instalacao!"
}

$PyFull = & $PythonExe -c "import sys; v=sys.version_info; print(f'{v.major}.{v.minor}.{v.micro}')"
...
if ($Extras.Count -gt 0) {
    ...
    RunCmd $VenvPython @("-m", "pip", "install") + $Extras + @("--quiet")
    ...
    if (Has "nvidia-smi") {
        try {
            ...
            $VramMb = (& nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits 2>$null |
                       ForEach-Object { [int]$_.Trim() } | Measure-Object -Sum).Sum
            $GpuVramGb = [math]::Round($VramMb / 1024, 1)
            $GpuType = "nvidia"
            # Obter versão CUDA
            $nvidiaSmiOut = & nvidia-smi 2>$null | Select-String "CUDA Version"
            if ($nvidiaSmiOut) {
                $CudaVersion = ($nvidiaSmiOut.Line -split "CUDA Version: ")[1].Split()[0]
            }
            Pass "NVIDIA GPU detectada: $GpuName  (VRAM: ${GpuVramGb} GB, CUDA: $CudaVersion)"
        } catch {
            Warn "nvidia-smi encontrado mas falhou ao consultar GPU: $_"
        }
    }
    # ── AMD (suporte experimental no Windows) ───────────────
    elseif (Has "rocm-smi") {
        $GpuType = "amd"
...
Write-Log "  GPU_TYPE=$GpuType  VRAM=${GpuVramGb}GB"
...
if ($GpuType -eq "nvidia") {
    $vllmInstalled = $false
    try {
        & $VenvPython -c "import vllm" 2>$null
    ...
    if (-not $vllmInstalled) {
        ...
        Warn "  Opcoes para modelos grandes:"
        Warn "  1. Use Ollama (recomendado) — suporta CUDA no Windows nativamente"
        Warn "  2. Use WSL2 + vLLM (para modelos 32B+):"
        Warn "       wsl --install -d Ubuntu-22.04"
        Warn "       # depois, dentro do WSL2, execute: bash setup.sh"
    ...
    Warn "  Configure WSL2: wsl --install -d Ubuntu-22.04"
    Warn "  Dentro do WSL2: instale ROCm e execute bash setup.sh"
    Pass "Backend CPU/Ollama (AMD requer WSL2 para aceleracao completa)"
} else {
    Info "GPU nao disponivel — vLLM nao sera instalado"
    Info "Modelos rodarao via Ollama em CPU"
...
if (-not (Has "node")) {
    ...
    Warn "  OU via scoop:  scoop install nodejs-lts"
    ...
    # No Windows, npm global instala em %APPDATA%\npm por padrão — sem necessidade de sudo
    $npmPrefix = (& npm config get prefix 2>$null).Trim()
    Info "npm prefix: $npmPrefix"

    # Garantir que o npm bin esteja no PATH da sessão
    $npmBin = Join-Path $npmPrefix ""
    if ($env:PATH -notlike "*$npmPrefix*") {
        $env:PATH = "$npmPrefix;$env:PATH"
        Info "npm prefix adicionado ao PATH da sessao"
        Info "Para persistir, adicione ao PATH do sistema: $npmPrefix"
    }
...
Info "Instalando browser Chromium com dependencias..."
try {
...
function New-GpuEnvBlock {
    $block = "`n# --- GPU Configuration (gerado pelo setup.ps1) ---`n"

    if ($GpuType -eq "nvidia") {
        $block += "# NVIDIA CUDA -- $GpuName  (${GpuVramGb} GB VRAM)`n"
        $block += "CUDA_VISIBLE_DEVICES=0`n"
        $block += "# Ollama usa CUDA automaticamente no Windows`n"
        $block += "# Para vLLM: instale no WSL2 e aponte VLLM_BASE_URL para localhost`n"
    } elseif ($GpuType -eq "amd") {
        $block += "# AMD GPU -- suporte experimental Windows`n"
        $block += "# Para ROCm completo: use WSL2`n"
        $block += "HIP_VISIBLE_DEVICES=0`n"
    } else {
        $block += "# CPU only -- GPU nao detectada`n"
        $block += "# Reduza paralelismo para evitar OOM`n"
        $block += "MAX_CONCURRENT_MODELS=1`n"
        $block += "MAX_CONCURRENT_SCANS=2`n"
    }
    return $block
}
...
if (Test-Path $EnvFile) {
    ...
    if ($envContent -match "GPU Configuration") {
        ...
        Add-Content -Path $EnvFile -Value (New-GpuEnvBlock)
...
"@ | Out-File -FilePath $EnvFile -Encoding UTF8
    ...
    Add-Content -Path $EnvFile -Value (New-GpuEnvBlock)
    Pass ".env criado com configuracao de GPU ($GpuType)"
}
...
if (-not (Has "ollama")) {
    ...
    # Criar arquivo de config do Ollama para Windows
    if (-not (Test-Path $OllamaEnvDir)) {
        New-Item -ItemType Directory -Path $OllamaEnvDir -Force | Out-Null
    }

    $ollamaEnvContent = "# Ollama environment -- gerado pelo a11y-autofix setup.ps1`n"
    $ollamaEnvContent += "# Windows: este arquivo e lido quando Ollama e iniciado`n`n"

    if ($GpuType -eq "nvidia") {
        $ollamaEnvContent += "# NVIDIA CUDA -- $GpuName  (${GpuVramGb} GB)`n"
        $ollamaEnvContent += "CUDA_VISIBLE_DEVICES=0`n"
        $ollamaEnvContent += "OLLAMA_GPU_OVERHEAD=256000000`n"
        $ollamaEnvContent += "OLLAMA_MAX_LOADED_MODELS=1`n"
        Pass "Ollama configurado para NVIDIA CUDA ($GpuName, ${GpuVramGb} GB)"
    } elseif ($GpuType -eq "amd") {
        $ollamaEnvContent += "# AMD GPU -- suporte experimental`n"
        $ollamaEnvContent += "HIP_VISIBLE_DEVICES=0`n"
        $ollamaEnvContent += "OLLAMA_MAX_LOADED_MODELS=1`n"
        Warn "Ollama configurado para AMD (experimental no Windows)"
    } else {
        $ollamaEnvContent += "# CPU only`n"
        $ollamaEnvContent += "OLLAMA_NUM_GPU=0`n"
        $ollamaEnvContent += "OLLAMA_NUM_PARALLEL=1`n"
        $ollamaEnvContent += "OLLAMA_MAX_LOADED_MODELS=1`n"
        Warn "Ollama configurado para CPU — inferencia sera lenta"
    }

    $ollamaEnvContent | Out-File -FilePath $OllamaEnvFile -Encoding UTF8
    Info "Config Ollama salva em: $OllamaEnvFile"

    # Instrucoes para variaveis de ambiente do sistema (Windows nao usa ollama.env da mesma forma)
    Write-Host ""
    Write-Host "  IMPORTANTE — Variaveis de ambiente do Ollama no Windows:" -ForegroundColor Yellow
    Write-Host "  O Ollama no Windows usa variaveis de ambiente do sistema."
    Write-Host "  Para aplicar a configuracao GPU, execute como Administrador:"
    Write-Host ""
    if ($GpuType -eq "nvidia") {
        Write-Host "    [System.Environment]::SetEnvironmentVariable('CUDA_VISIBLE_DEVICES','0','Machine')" -ForegroundColor Cyan
        Write-Host "    [System.Environment]::SetEnvironmentVariable('OLLAMA_GPU_OVERHEAD','256000000','Machine')" -ForegroundColor Cyan
    } else {
        Write-Host "    [System.Environment]::SetEnvironmentVariable('OLLAMA_NUM_GPU','0','Machine')" -ForegroundColor Cyan
    }
    ...
    try {
        $response = Invoke-WebRequest -Uri "http://localhost:11434/" -TimeoutSec 2 -UseBasicParsing 2>$null
        Info "Ollama daemon esta rodando"
    } catch {
        Info "Ollama daemon nao esta rodando"
        Info "  Inicie com: ollama serve"
        Info "  OU abra o aplicativo Ollama na bandeja do sistema"
    }
}
...
if (-not (Has "ollama")) {
    ...
    if (-not $ollamaRunning) {
        ...
        Warn "  Execute depois:"
        Warn "    ollama serve"
        Warn "    ollama pull qwen2.5-coder:7b"
    } elseif ($NoModels) {
        Info "Pull desativado via -NoModels"
    } else {
        $ModelsToPull = @()

        if ($GpuType -eq "none" -or $NoGpu) {
            ...
            Info "VRAM ${GpuVramGb} GB -> baixando modelos ate 14B"
        } elseif (VramGte 6) {
            $ModelsToPull = @("qwen2.5-coder:7b")
            Info "VRAM ${GpuVramGb} GB -> baixando apenas modelo 7B"
        } else {
            $ModelsToPull = @("qwen2.5-coder:7b")
        }

        $Available = & ollama list 2>$null | Select-Object -Skip 1 | ForEach-Object { ($_ -split "\s+")[0] }
...
$a11yAutofix = Join-Path $VenvDir "Scripts\a11y-autofix.exe"
if (-not (Test-Path $a11yAutofix)) {
    $a11yAutofix = Join-Path $VenvDir "Scripts\a11y-autofix"
}

if (Test-Path $a11yAutofix) {
    Info "Executando a11y-autofix hardware..."
    try {
        RunVisible $a11yAutofix @("hardware")
...
Write-Host "=" * 60
Write-Host "  Passou  : $NPass" -ForegroundColor Green
if ($NWarn -gt 0) { Write-Host "  Avisos  : $NWarn" -ForegroundColor Yellow }
if ($NFail -gt 0) { Write-Host "  Falhas  : $NFail" -ForegroundColor Red }
Write-Host ""
Write-Host "SO/GPU:"
Write-Host "  Sistema : Windows"
switch ($GpuType) {
    "nvidia" { Write-Host "  GPU     : NVIDIA $GpuName  (${GpuVramGb} GB VRAM, CUDA $CudaVersion)" -ForegroundColor Green }
...
Write-Host "  a11y-autofix hardware                             # verificar hardware"
Write-Host "  a11y-autofix models list                          # listar modelos"
Write-Host "  a11y-autofix fix .\src --dry-run                  # scan de teste"
Write-Host "  a11y-autofix experiment run experiments\base.yaml # experimento"
Write-Host ""
Write-Host "Log completo: $LogFile" -ForegroundColor DarkGray
Write-Host ""

if ($NFail -gt 0) {
    Write-Host "Setup concluido com $NFail falha(s). Verifique as mensagens acima." -ForegroundColor Red
    exit 1
} elseif ($NWarn -gt 0) {
    Write-Host "Setup concluido com avisos. Experimento pode rodar com funcionalidade reduzida." -ForegroundColor Yellow
} else {
    Write-Host "Ambiente configurado com sucesso para o experimento!" -ForegroundColor Green
}
